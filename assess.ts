...
function calculateRiskAssessment(data: Assessment): AssessmentApiResponse {
  ...
  if (data.locationAccess) {
    ...
    if (data.locationAccess.latitude && data.locationAccess.longitude) {
      ...
      const searchTerm =
        data.locationAccess.region || data.locationAccess.province || data.locationAccess.city || "";
    ...
    if (urgency === "urgent") {
      nearbyFacilities = nearbyFacilities.filter(
        (f) => f.type === "hospital" || f.type === "emergency"
      );
    }
  ...
  return {
    ...
    meta: {
      ...
      locationProvided,
      facilityNoteKey:
        recommendedFacilities && recommendedFacilities.length > 0
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const response = calculateRiskAssessment(validationResult.data);
